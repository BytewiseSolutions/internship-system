...
try {
    ...
    // Get accepted internship with company details
    $stmt = $conn->prepare("
        SELECT 
            a.application_id,
            a.status,
    ...
    if ($result->num_rows === 0) {
        send_json(['success' => true, 'internship' => null, 'message' => 'No accepted internship found']);
    } else {
        $internship = $result->fetch_assoc();
        send_json(['success' => true, 'internship' => $internship]);
    }
